...
exports.deploy = function(pm, options) {
	...
	function deploy(hostname) {
		...
		SM.for(pm.context.program.package.path).require({
			...
			pm.context.credentials.requestFor("git", "user.name").then(function(GitUserName) {
				return pm.context.credentials.requestFor("git", "user.email").then(function(GitUserEmail) {
					return pm.context.credentials.requestFor("github.com", "username").then(function(GithubUsername) {
	...
	function ensureInstanceTags() {
		...
		ec2.call("DescribeTags", {
			...
			if (!existingTags["pinf:user"]) {
					...
					return pm.context.credentials.requestFor("github.com", "username").then(function(Username) {
	...
	function ensureSecurityGroup() {
		...
		return Q.when(pm.context.deploymentDescriptor.get(["config", "github.com/sourcemint/sdk-aws/0", "SecurityGroup"])).then(function(SecurityGroup) {
			...
			if (SecurityGroup === null) {
				...
				SecurityGroup = SecurityGroup || config.SecurityGroup || pm.context.credentials.requestFor("github.com", "username").then(function(Username) {
	...
	function createInstance(KeyName) {
		...
		return Q.when(pm.context.deploymentDescriptor.get(["config", "github.com/sourcemint/sdk-aws/0", "SecurityGroup"])).then(function(SecurityGroup) {
			...
			TERM.stdout.writenl("Launching AWS instance based on AMI '" + config.ImageId + "' under security group '" + SecurityGroup + "'.");
	...
	return pm.context.credentials.requestFor("aws.amazon.com", "KeyName").then(function(KeyName) {
		...
		if (pm.context.deploymentDescriptor.has(["config", "github.com/sourcemint/sdk-aws/0", "InstanceId"])) {
			// TODO: Ensure `InstanceId` actually exists by checking with AWS.
			...
			return ensureSecurityGroup().then(function() {
				return createInstance(KeyName).then(prepare);
			});
		}
	});
	});
